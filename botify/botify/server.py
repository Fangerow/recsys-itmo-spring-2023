...
from botify.recommenders.custom_rec import ContextualCustom
...
tracks_redis_custom = Redis(app, config_prefix="REDIS_TRACKS_CUSTOM")
...
custom_catalog = Catalog(app).load(
    app.config["TRACKS_CATALOG_CUSTOM"], app.config["TOP_TRACKS_CATALOG"]
)
custom_catalog.upload_tracks(tracks_redis_custom.connection,)
...
class NextTrack(Resource):
    ...
    def post(self, user: int):
        ...
        treatment = Experiments.CUSTOM_CONTEXTUAL.assign(user)
        if treatment == Treatment.T1:
            recommender = ContextualCustom(
                tracks_redis_custom.connection,
                custom_catalog,
